...
namespace Kusume
{
    ...
    public class StateMachine<TKey> : IStateMachine, IStateChanger<TKey>
    {
        ...
        //当たり判定の当たり初めに使う関数
        public void DoTriggerEnter(GameObject thisObject, Collider2D collider) => currentState?.DoTriggerEnter(thisObject, collider);
        //当たり判定の当たり続けている時に使う関数
        public void DoTriggerStay(GameObject thisObject, Collider2D collider) => currentState?.DoTriggerStay(thisObject, collider);
        //当たり判定の当たり終わり時に使う関数
        public void DoTriggerExit(GameObject thisObject, Collider2D collider) => currentState?.DoTriggerExit(thisObject, collider);
        //状態を遷移するための条件を追加する関数
